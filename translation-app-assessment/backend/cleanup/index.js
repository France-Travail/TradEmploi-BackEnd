...
app.post("/", async (req, res) => {
  ...
  const deletionTimeThreshold = parseInt(
    Moment().subtract(1, "hour").format("x")
  )
  const querySnapshot = await collection
    .where("expiryDate", "<", deletionTimeThreshold)
    .get()
...
async function cleanRates() {
   ...
   * Delete rates created more than 1 year ago
   */
  ...
  const tooOldRatesQuerySnapshot = await collection
  ...
  for (const docSnapshot of tooOldRatesQuerySnapshot.docs) {
    deletionPromises.push(collection.doc(docSnapshot.id).delete())
  }
  await Promise.all(deletionPromises)
}
...
async function kpi() {
  ...
  await roomsReference.get().then((querySnapshot) => {
...
const getDuration = (lastMessageTime, firstMessageTime) => {
  ...
  return `${formatNumber((nbSeconds / 3600) | 0)}h${formatNumber(
    ((nbSeconds % 3600) / 60) | 0
  )}`
}

const getTranslationMode = (messages) => {
  const mode = ["Texte", "Voix et Texte", "Voix"]
  let i = 1
  const textNone =
    messages.find((message) => message.translationMode !== "Texte") !==
    undefined
  const vocalNone =
    messages.find((message) => message.translationMode !== "Voix") !== undefined
  if (textNone) {
    i += 1
  }
  if (vocalNone) {
...
const formatTime = (time) => {
  ...
  return `${formatNumber(Number(timeArray[0]) | 0)}h${formatNumber(
...
async function createLanguagesFromRates() {
  ...
  const mapLanguages = languagesSelected
    .filter((l) => l)
    .reduce((acc, e) => acc.set(e, (acc.get(e) || 0) + 1), new Map())
  const languagesSorted = new Map(
    [...mapLanguages.entries()].sort((a, b) => b[1] - a[1])
  )
  Array.from(languagesSorted.keys()).forEach((isoCode) => {
    createLanguage(
      isoCode,
      languagesSorted.get(isoCode),
      langaugesAverageRate.get(isoCode)
    )
  })
