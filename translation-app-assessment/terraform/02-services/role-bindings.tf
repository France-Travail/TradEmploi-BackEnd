...
# Allow detect text service to use the Cloud Storage API
...
resource "google_cloud_run_service_iam_member" "detect-text" {
  location = google_cloud_run_service.detect_text.location
  project = google_cloud_run_service.detect_text.project
  service = google_cloud_run_service.detect_text.name
  role = "roles/run.invoker"
  member = "serviceAccount:${google_service_account.api_gateway_sa.email}"
}
